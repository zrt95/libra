...
#[derive(Debug)]
pub struct TransactionParameters(Vec<AccountAddress>);

impl TransactionParameters {
    pub fn new_from(txn: &Transaction) -> Self {
...
impl DependencyAnalyzer {
            ...
            if let Transaction::UserTransaction(user_txn) = txn {
                if let TransactionPayload::Script(script) = user_txn.payload() {
                    ...
                    if !read_write_infer.contains_key(script.code()) {
                        ...
                        if let Ok((_, output, _)) = diem_vm.execute_single_transaction(
                            &verif_txn,
    ...
    > {
        if let PreprocessedTransaction::UserTransaction(user_txn) = txn {
            if let TransactionPayload::Script(script) = user_txn.payload() {
                let deps = self.script_map.get(script.code()).unwrap();
